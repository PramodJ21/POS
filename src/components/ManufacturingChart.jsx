...
import styles from './ManufacturingChart.module.css'
const ManufacturingChart = ({manufacturingChartData}) => {
    const sortedData = manufacturingChartData.sort((a, b) => new Date(a.manufacturingEndDate) - new Date(b.manufacturingEndDate));

    const chartData = {
      labels: sortedData.map((entry) => entry.manufacturingEndDate), // Dates in ascending order
      datasets: [
        {
          label: 'Quantity Manufactured',
          data: sortedData.map((entry) => entry.totalQuantity), // Quantities
